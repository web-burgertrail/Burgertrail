...
// Swiggy-style: icon + label inside a pill. Active = orange filled pill, lifts up.
const NAV_TABS = [
  {
    path: '/',
    label: 'Home',
    icon: (active) => (
      <svg width="18" height="18" viewBox="0 0 24 24" fill={active ? 'white' : 'none'} stroke={active ? 'white' : 'rgba(255,255,255,0.45)'} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
...
function StoreTimerBar() {
  ...
  return (
    <div className={`w-full py-1.5 px-4 text-center flex items-center justify-center gap-2 ${
      isOpen ? 'bg-green-900/50 border-b border-green-700/30' : 'bg-dark-800/80 border-b border-orange-primary/15'
    }`}>
    ...
    <div className="fixed top-0 left-0 right-0 z-50 bg-dark-900/97 backdrop-blur-xl">
      ...
      <div className="flex items-center justify-between px-3 sm:px-5 lg:px-10 py-2 border-b border-white/5">
        ...
        <Link to="/" onClick={handleHomeClick} className="flex items-center gap-2 group flex-shrink-0">
          ...
          <span className="font-display" style={{ fontSize: 'clamp(0.85rem, 3vw, 1.05rem)' }}>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          {/* Instagram */}
          <a href="https://www.instagram.com/burgertrails" target="_blank" rel="noopener noreferrer" title="Instagram"
      ...
      {/* ── ROW 3: SWIGGY-STYLE PILL NAV ── */}
      {/*
        Swiggy exact design:
        - All tabs sit on a baseline
        - Active tab gets a colored pill/bubble background
        - Active tab slightly "lifts" (translateY -2px)
        - Active icon + label both turn white (inside orange pill)
        - Inactive: icon only, no label visible, muted color
              ...
              return (
                ...
                >
                  <motion.div
                    animate={{
                      y: active ? -2 : 0,
                      backgroundColor: active ? '#E4572E' : 'transparent',
                    }}
                    ...
                    style={{
                      ...
                      borderRadius: '999px',
                      cursor: 'pointer',
                      // Swiggy active shadow
                      boxShadow: active ? '0 4px 12px rgba(228,87,46,0.45)' : 'none',
                    }}
                  >
                    {/* Icon */}
                    <motion.span
                    ...
                    {/* Label — only visible when active (Swiggy style) */}
                    <motion.span
                      animate={{
                        width: active ? 'auto' : 0,
                        opacity: active ? 1 : 0,
