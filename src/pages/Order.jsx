...
export default function Order() {
  ...
  const handleManualOrder = () => {
    ...
    sendToWhatsApp(buildMessage(`📍 My Address:\n${addr}`));
  };
    ...
    >
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <h1 className="font-display text-6xl sm:text-7xl tracking-widest text-white">
            YOUR <span className="text-orange-primary">CART</span>
          </h1>
        </motion.div>
        ...
        {items.length === 0 && !ordered && (
          ...
          >
            ...
            <p className="text-white/20 font-body text-sm mt-2 mb-8">
              Add some delicious items from our menu
            </p>
            <Link
              to="/menu"
              className="inline-flex items-center gap-2 bg-orange-primary hover:bg-orange-light text-white px-8 py-3.5 rounded-full font-heading font-medium tracking-wide transition-all duration-300 glow-orange hover:scale-105"
            >
              Browse Menu
        ...
        {ordered && (
          ...
          >
            ...
            <div className="flex flex-wrap justify-center gap-4">
              <button
                onClick={() => { clearCart(); setOrdered(false); }}
                className="glass px-8 py-3.5 rounded-full font-heading font-medium tracking-wide text-sm hover:border-orange-primary/40 hover:text-orange-primary transition-all"
              >
                Clear & Start Over
              </button>
              <Link
                to="/menu"
                className="bg-orange-primary hover:bg-orange-light text-white px-8 py-3.5 rounded-full font-heading font-medium tracking-wide text-sm transition-all glow-orange hover:scale-105"
              >
                Order More
              </Link>
            </div>
        ...
        {items.length > 0 && !ordered && (
          <div className="grid lg:grid-cols-3 gap-8">
              ...
              <AnimatePresence>
                {items.map((item) => (
                  <motion.div
                    ...
                    className="glass rounded-2xl p-5 flex gap-5 hover:border-orange-primary/20 transition-all duration-300"
                  >
                    {/* Image */}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-orange-primary font-body text-sm mt-0.5">₹{item.price} each</p>
                      ...
                      <div className="flex items-center gap-3 mt-3">
                        <div className="flex items-center gap-2 glass rounded-full px-3 py-1.5">
                          <button
                            ...
                            className="w-6 h-6 flex items-center justify-center text-white/60 hover:text-orange-primary transition-colors font-bold text-lg leading-none"
                          >
                            −
                          </button>
                          <span className="text-white font-heading text-sm w-5 text-center">
                            {item.quantity}
                          </span>
                          <button
                            onClick={() => updateQty(item.id, item.quantity + 1)}
            ...
            <div className="lg:col-span-1">
              <div className="glass-orange rounded-2xl p-6 sticky top-24 space-y-5">
                ...
                <div className="space-y-2">
                  {items.map(i => (
                    <div key={i.id} className="flex justify-between text-sm font-body">
                      <span className="text-white/50 truncate flex-1 pr-2">
                        {i.quantity}× {i.name}
                      </span>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-orange-primary font-display text-3xl tracking-wider">
                    ₹{total}
                  </span>
                </div>
                ...
                <motion.button
                  ...
                  onClick={handleOrder}
                  disabled={loading}
                  className="w-full bg-orange-primary hover:bg-orange-light text-white py-4 rounded-full font-heading font-medium tracking-wide transition-all duration-300 glow-orange flex items-center justify-center gap-2 disabled:opacity-70"
                >
                  {loading ? (
                    <>
                  ...
                  {locError && (
                    ...
                    >
                      <p className="text-orange-primary text-xs font-body">
                        📍 Location denied. Enter your address:
                      </p>
                      <input
                        type="text"
                        value={manualAddress}
                ...
                <p className="text-white/20 text-xs font-body text-center">
                  Orders via WhatsApp • Delivery in Upperpally area
                </p>
              </div>
            </div>
          </div>
